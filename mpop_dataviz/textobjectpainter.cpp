...
TextObjectPainter::TextObjectPainter() {
    ...
    // Here, we populate the text labels. (deprecated)
    // Moyenne text
...
void TextObjectPainter::drawViewElements(ViewModeManager::ViewMode view, const QList<QString> &title) {
    ...
    // Function to draw the range text.
    auto drawMinMaxBottomText = [&]() {
        _painter.setFont(_smallFont);
        qreal bottomTitlePosY = _height - fitToScreenHeight(hLineMrgBottom - 32);
        // We use the _minLabel and _maxLabel text.
        _painter.drawText(97, bottomTitlePosY, _minLabel);
        _painter.drawText(_width - (hLineMrgRight + 75), bottomTitlePosY, _maxLabel);
    ...
    // Function to draw the % and 0-100 the bottom.
    //auto drawPercentBottomText = [&]() {
